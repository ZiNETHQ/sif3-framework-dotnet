...
{
    public class RightsConverter : ITypeConverter<rightType[], IDictionary<string, Right>>
    {
        public IDictionary<string, Right> Convert(rightType[] source, IDictionary<string, Right> destination, ResolutionContext context)
        {
            ICollection<Right> values = context.Mapper.Map<rightType[], ICollection<Right>>(source);
